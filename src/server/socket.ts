...
io.on('connection', (socket) => {
  ...
  socket.on(
    'start-transcription',
    async (data: { transcriptionId: string; userId: string; jwt: string }) => {
      try {
  ...
  socket.on('audio-data', async (audioData) => {
    try {
      ...
      fetch(
        ...
        .then((data) => {
          // Processamento direto sem arquivo temporário
    ...
    try {
      ...
      await supabase
        ...
        .update({
          ...
          duration_seconds:
            segments.length > 0 ? Math.max(...segments.map((s: any) => s.end), 0) : 0,
        })
        .eq('id', transcriptionId);
