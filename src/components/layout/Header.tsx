...
export function Header({ showNewButton = false, rightContent, locale, onToggleLocale }: HeaderProps) {
  ...
  return (
    <header className="sticky top-0 z-50 w-full border-b border-neutral-200 bg-white/90 backdrop-blur-xl">
      <div className="max-w-6xl mx-auto px-4 sm:px-8 flex h-14 items-center justify-between gap-2">
        <Link href="/" className="flex min-w-0 items-center gap-2.5 font-semibold tracking-tight text-neutral-900">
          <img src="/logo.png" alt="Logo" className="h-7 w-7 flex-shrink-0 rounded-lg object-cover" />
          <span className="hidden truncate sm:inline">MeetingsTranscript</span>
        </Link>
